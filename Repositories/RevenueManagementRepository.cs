...
public sealed class RevenueManagementRepository(ApplicationDbContext db) : IRevenueManagementRepository
{
    public async Task<PagedResult<UserCourseAssignmentDto>> SearchAssignmentsAsync(DataRequest request, CancellationToken ct)
    {
        var query = db.UserCourseAssignments.AsNoTracking().Where(x => !x.IsDeleted).ApplyFilters(request.Filters);
        var total = await query.CountAsync(ct);
        query = query.ApplySorting(request.Sort).ApplyPagination(request.Pagination);
        var items = await query.Select(x => new UserCourseAssignmentDto(x.Oid, x.UserId, x.User.Username,
            x.CourseId, x.Course.CourseName, x.AssignmentTypeLookupId,
            x.AssignmentType.LookupNameEn ?? x.AssignmentType.LookupValue, x.IsPrimary, x.IsActive)).ToListAsync(ct);
        return PagedResult<UserCourseAssignmentDto>.Create(items, total, request.Pagination.PageNumber, request.Pagination.PageSize);
    }

    public Task<UserCourseAssignmentDto?> GetAssignmentByIdAsync(Guid id, CancellationToken ct) => GetAssignmentAsync(id, ct);

    public async Task<PagedResult<CourseRevenueShareDto>> SearchSharesAsync(Guid courseId, DataRequest request, CancellationToken ct)
    {
        var query = db.CourseRevenueShares.AsNoTracking().Where(x => x.CourseId == courseId && !x.IsDeleted).ApplyFilters(request.Filters);
        var total = await query.CountAsync(ct);
        query = query.ApplySorting(request.Sort).ApplyPagination(request.Pagination);
        var items = await query.Select(x => new CourseRevenueShareDto(x.Oid, x.CourseId, x.BeneficiaryUserId,
            ...
            x.IsActive, x.EffectiveFrom, x.EffectiveTo, x.Notes)).ToListAsync(ct);
    ...
    public async Task<PagedResult<RevenueSettlementDto>> SearchSettlementsAsync(DataRequest request, CancellationToken ct)
    {
        var query = db.RevenueSettlements.AsNoTracking().Where(x => !x.IsDeleted).ApplyFilters(request.Filters);
        var total = await query.CountAsync(ct);
        query = query.ApplySorting(request.Sort).ApplyPagination(request.Pagination);
        var items = await query.Select(x => new RevenueSettlementDto(x.Oid, x.BeneficiaryUserId,
            x.BeneficiaryUser.Username, x.SettlementNumber, x.PeriodFrom, x.PeriodTo, x.TotalAmount,
            x.Status, x.PaidAt, x.PaymentReference, x.Notes)).ToListAsync(ct);
        return PagedResult<RevenueSettlementDto>.Create(items, total, request.Pagination.PageNumber, request.Pagination.PageSize);
    }

    public Task<RevenueSettlementDto?> GetSettlementByIdAsync(Guid id, CancellationToken ct) => GetSettlementAsync(id, ct);
