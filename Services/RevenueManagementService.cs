...
public sealed class RevenueManagementService(IRevenueManagementRepository repository) : IRevenueManagementService
{
    ...
    public async Task<PagedResponse<CourseRevenueDetailsDto>> SearchCourseRevenueAsync(DataRequest request, Guid userId, bool globalAccess, CancellationToken ct) => ToPagedResponse(await repository.SearchCourseRevenueAsync(request, userId, globalAccess, ct));
